...
import { ElementLibrary, elementMap } from './Components/ElementLibrary';
...
const Home = ({ setCanvasHTML }) => {
  ...
  const handleDrop = (e) => {
    ...
    if (items) {
      ...
      const newElement = { items, style: 'margin: 10px;' };
      const updatedElements = [...elements, newElement];
      setElements(updatedElements);
    }
  };

  const handleCanvasClick = (e, elem) => {
    e.stopPropagation();
    if (preview) return; // Disable editing in preview mode
  ...
  const updateElementProps = (newProps) => {
    setCanvasElements((prev) =>
      prev.map((e) => (e.id === selectedElement.id ? { ...e, props: newProps } : e))
    );
  };
  ...
  return (
    <div className="mt-16">
            ...
            >
              ...
              <g id="SVGRepo_iconCarrier">
                <g id="XMLID_334_">
                  ...
                  ></path>
                  <circle id="XMLID_888_" cx="119.083" cy="174.96" r="34.123"></circle>
                  <circle id="XMLID_889_" cx="230.837" cy="174.96" r="34.123"></circle>
                </g>
              </g>
            </svg>
          </a>
      ...
      <div className="max-w-[95vw] flex ml-16">
        ...
        {/* CANVAS */}
        <div className={preview ? previewContainerStyle : canvasContainerStyle} ref={canvasRef}>
          <div
            id="canvas"
            className={`${preview ? previewStyle : canvasStyle} flex flex-wrap gap-1 items-start`}
            ...
            {canvasElements.map((elem) => {
              ...
              return (
                <div
                  key={elem.id}
                  style={elem.style}
                  onClick={(e) => handleCanvasClick(e, elem)}
                  className={selectedElement?.id === elem.id ? 'border-2 border-blue-500' : ''}
                >
                  <Component
                    {...elem.props}
                    style={{ ...elem.style, ...elem.props.style }}
                    onUpdate={(updateProps) =>
                      setCanvasElements((prev) =>
                        prev.map((e) => (e.id === elem.id ? { ...e, props: updateProps } : e))
                      )
                    }
                  />
                </div>
          ...
          {selectedElement && (
            <div style={{ position: 'absolute', left: editPosition.y, top: editPosition.y }}>
              <EditText
